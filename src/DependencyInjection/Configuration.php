...
class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder()
    {
        ...
        $treeBuilder->getRootNode()
            ->children()
                ->scalarNode('csv_path')->defaultNull()->info('Path relative to project root to csv file containing inventory stock data to be imported.')->end()
                ->arrayNode('out_of_stock_email')
                    ->children()
                        ->scalarNode('to')->defaultNull()
                            ->info('Email address where to send the out of stock notification. Required in order for notifications to work.')
                        ->end()
                        ->scalarNode('from')->defaultNull()
